...
function toggleTheme() {
    ...
    for (const id of ['themeIcon', 'gameThemeIcon', 'gearThemeIcon']) {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Init settings tags
    updateSettingsTags();
    // Init animated background (runs behind auth screen too)
    initBgCanvas();

    // Close gear menu when clicking outside
    document.addEventListener('click', e => {
        const wrap = document.getElementById('fixedActions');
        if (wrap && !wrap.contains(e.target)) closeFixedGear();
    }, true);

    // Hash routing: if URL already has #login on load, show auth screen
    if (location.hash === '#login') goToLogin();
});

// Called by firebase-auth.js once the user is confirmed signed-in
window.onAuthReady = function(user) {
    const name = user?.displayName || '';
    const createInput = document.getElementById('playerNameCreate');
    const joinInput   = document.getElementById('playerNameJoin');
    if (createInput && !createInput.value && name) createInput.value = name;
    if (joinInput   && !joinInput.value   && name) joinInput.value   = name;
};
...
function toggleFixedGear() {
    const btn  = document.getElementById('fixedGearBtn');
    const menu = document.getElementById('fixedGearMenu');
    if (!btn || !menu) return;
    const open = btn.classList.toggle('gear-open');
    menu.classList.toggle('gear-open', open);
}

function closeFixedGear() {
    document.getElementById('fixedGearBtn')?.classList.remove('gear-open');
    document.getElementById('fixedGearMenu')?.classList.remove('gear-open');
}
